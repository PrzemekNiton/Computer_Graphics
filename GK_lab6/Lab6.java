...
public class Lab6 extends GLJPanel implements GLEventListener  {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        float[] ambientTier = { 0.77f, 0.77f, 0.77f, 1f };
        gl2.glLightModelfv(GL2.GL_LIGHT_MODEL_AMBIENT,ambientTier, 0);
    ...
    public void DrawShape(GL2 gl2, int pyramidCorners) {
        double radius = 6.5;
        float z = 0;
        float x = 0;
        double shapeAngle = 0; //zmienna pomocnicza, kąt początkowy
        double angleGrowth = 2 * Math.PI / pyramidCorners;
        float[] material = materials[7];

        shapeAngle = 0;
        //pętla tworzy wielokąty
        for (int i = 1; i <= pyramidCorners; i++) {

            gl2.glBegin(GL2.GL_POLYGON);   //funkcja rozpoczyna utworzenie trójkątów
            x = (float) (radius * Math.cos(shapeAngle));
            z = (float) (radius * Math.sin(shapeAngle));
            shapeAngle = i * angleGrowth;

            //utworzenie trojkatów
            ...
            gl2.glVertex3f((float) (radius * Math.cos(shapeAngle)), -2.0f,(float) (radius * Math.sin(shapeAngle)));
