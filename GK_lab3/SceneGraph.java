...
public class SceneGraph extends JPanel {
	...
	private static abstract class SceneGraphNode {
		...
		abstract void DrawAdd(Graphics2D g);
	}
	...
	private static SceneGraphNode filledTriangle = new SceneGraphNode() {
		void DrawAdd(Graphics2D g) { // width = 1, height = 1, center of base is at (0,0);
	...
	private static SceneGraphNode filledPolygon = new SceneGraphNode() {
		...
		void DrawAdd(Graphics2D g2) {
			int l = 5;

			double r = 150, t = 0, k = (Math.PI * 2) / l;
			int[] x1 = new int[l];
			int[] y1 = new int[l];
			for (int i = 0; i < l; i++) {
			...
			Polygon polygon = new Polygon(x1, y1, l);
